...
- Added "Only specified" toggle to Mask Painter (#89 follow-up): a new `only_specified_checkbox` placed inline with the default color picker; when ON, the active class list is filtered to classes whose assigned color differs from the default color — classes still at the default are moved to `available_classes` and receive the `""` sentinel on the next apply; when OFF, all `current_classes` are restored to active.
- Added Add/Remove class feature to Mask Painter (#89 follow-up): users can now start with the first ≤6 classes active and dynamically add or remove classes via inline JS controls (a `<select>` + `+ Add` button in a footer row; `×` per-row buttons to remove). Inactive classes are stored in `available_classes` and receive the `""` sentinel on the next apply. Implemented via three new traitlets (`available_classes`, `add_requested`, `remove_requested`) and corresponding Python observers (`_on_add_requested`, `_on_remove_requested`).
