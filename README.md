...
- Fixed scatter plots cropping points and not filling the width when plotting many pairs (#118): plotting several channels at once used to clip data points near the edges and leave each plot squeezed into part of its space (with a mismatched, doubled-up axis grid), and each plot opened at an inconsistent zoom with its right-side y-axis ticks/labels hidden until you clicked "reset view". Now, when multiple scatter plots are shown in the footer, the **controls sit on top and the plots take the full viewer width**, the pairwise matrix uses a proper grid so **each plot fills its cell with a single, correctly-scaled axis grid**, every scatter reserves a small margin around the data, and **each plot resets its view automatically as soon as it appears** — so points near the axis edges are no longer cut off, the axes/ticks are drawn correctly, and no "reset view" click is needed.
