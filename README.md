...
## New Update  
### **UELer v0.4.3 Summary**
- **AnnData cell tables are supported directly (#123).** You can now hand UELer an `AnnData` object (or an `.h5ad` path) instead of a pandas DataFrame — no more flattening it by hand first:
  ```python
  import anndata as ad
  from ueler import load_cell_table

  load_cell_table(viewer, cell_table=ad.read_h5ad("cells.h5ad"))
  # or straight from disk:
  load_cell_table(viewer, cell_table_path="cells.h5ad")
  ```
  Your object is **kept**, not converted away: `obs` columns and the expression matrix (`var_names`) both become selectable columns everywhere in the UI, embeddings stored in `obsm` (e.g. `X_umap`) show up as `X_umap1`/`X_umap2` in the scatter axes, and markers are listed **first** in the marker pickers instead of after `label`/`area`/`X`/`Y`. Anything the plugins add afterwards — FlowSOM clusters, heatmap meta-clusters, manual labels from the Cell Table Editor — is written back into `adata.obs`, so `viewer.get_cell_table_adata().write_h5ad("annotated.h5ad")` gives you your object with the new annotations in it. Use `layer="counts"` to read a `layers` entry instead of `X`. Two bugs that only surfaced with `.h5ad` input were fixed along the way: string `obs` columns (which `write_h5ad` stores as categoricals) used to be missing from every class/cluster dropdown, and assigning a brand-new class in the Cell Table Editor used to fail outright.
- **Marker/feature selection no longer hides its options (#125).** The channel/marker pickers — in the left panel and in the Scatter plot, Histogram, Heatmap and FlowSOM plugins — used to rely on the browser's native suggestion popup, which capped how many markers it showed, could not be scrolled, was cut off at the panel edge, and only appeared while you were typing. They are now a **searchable picker with a scrollable option list**: click the field to browse **every** available marker, type to filter (case-insensitive), use **Select all shown** to grab a whole filtered group at once (e.g. type `CD`), remove single selections from their chips, and navigate with the keyboard (↑/↓, Enter to toggle, Esc to close, Backspace to drop the last one). The list is rendered inside the panel, so it can never be clipped — including in VS Code notebooks and other embedded viewers.
- **A plot selection stays highlighted when you switch FOV (#119).** Cells selected in the **Scatter plot** or **Histogram** (with **Main viewer** linked) used to lose their outlines in the image the moment you moved to the next FOV — you had to re-draw the lasso or re-brush the range for every FOV. The selection is a set of cells, not of pixels, so it now follows you: switch FOV in **Select Image** and the selected cells belonging to the newly loaded FOV are outlined immediately, in both directions and however many times you switch. A FOV that contains none of the selected cells simply shows nothing until you move to one that does. The outlines also **stay put while you zoom and pan**: they used to flash and then vanish a moment later, because the newly rendered image was drawn on top of them. Selections drawn directly in the image (a click or a **lasso**) are spatial and still stay with the FOV they were made in, and the histogram cutoff / heatmap cluster highlights behave exactly as before.
- **Selected channels can be dragged into any order (#126).** The chips showing your current selection are draggable again — grab a chip by its `⋮⋮` grip and drop it where you want; a blue bar shows exactly where it will land, and dropping past the last chip moves it to the end. You can also focus a chip and use ← / → to move it (or Delete to remove it), which works in hosts where drag-and-drop is intercepted. The new order is applied straight away to the **channel colour and contrast controls** below the picker, which reorder to match while keeping each channel's colour and Min/Max settings, and to the order the channels are composited in. This works in the left-panel **Channels** picker and in the Scatter plot, Histogram, Heatmap and FlowSOM marker pickers.
- **Legacy `viewer.*` imports work again, and the test suite is green.** After the move to the `ueler` package, notebooks that still did `from viewer.main_viewer import ImageMaskViewer` failed outright — the compatibility shim had not been updated for the completed migration. The whole legacy namespace is now mapped onto the new one, so any `viewer.<module>` import resolves to exactly the same module as its `ueler.viewer.<module>` counterpart (run `import ueler` once first). Two rendering/robustness bugs surfaced by the same pass were fixed as well: mask outlines no longer draw a frame around the rendered region on installations without scikit-image, and a batch FOV export can no longer report success when nothing was written to disk. The suite now runs clean at 737 tests (previously 31 stale failures).
- The **Scatter plot**, **Chart (heatmap)**, and **Heatmap** plugins now live permanently in the wide footer panel below the image (#121): they no longer jump between the side panel and the footer, so the layout stays consistent and the plots always have the full viewer width to work with. The Heatmap in particular now always renders in its wide (horizontal) orientation and its `Horizontal layout` checkbox is gone. The **Histogram** remains in the side panel. The Heatmap also **fills the full footer width on load** and remembers the size after you drag its resize handle.

_Earlier changes (v0.4.2 and before) are in the [update log](/doc/log.md)._
