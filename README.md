...
- Fixed scatter plots cropping points and not filling the width when plotting many pairs (#118): plotting several channels at once used to clip data points near the edges and leave each plot squeezed into part of its space (with a mismatched, doubled-up axis grid). Now, when multiple scatter plots are shown in the footer, the **controls sit on top and the plots take the full viewer width**, the pairwise matrix uses a proper grid so **each plot fills its cell with a single, correctly-scaled axis grid**, and every scatter reserves a small margin around the data — so points near the axis edges are no longer cut off.
- Load marker sets in the heatmap, just like the scatter plot (#117): the **Heatmap** plugin's channel picker now matches the **Scatter plot** and **Histogram** plugins — a **"Marker set:"** dropdown and **"Load set"** button sit beside the channel tags, so you can load a predefined marker set (defined and saved in the left panel) into the heatmap with one click instead of adding each marker by hand. Loading a set only fills the heatmap's own picker; it does not change the main image view.
- Show more images at full resolution before downsampling (#116): the viewer used to start downsampling frames once their longest edge passed ~512 px, which unnecessarily softened lower-resolution images. The threshold is now **2048 px**, so frames up to that size (and viewport regions you zoom into up to that size) render at native resolution; downsampling only kicks in for larger frames. The **initial** view now also respects the **Downsample** checkbox — when it is off, a freshly-opened FOV starts at native resolution instead of being downsampled on load (previously only zooming applied that setting).
- Made continuous mask coloring fast and added a busy indicator (#115 reply): applying a continuous gradient used to take minutes and freeze the app with no progress shown. It now renders in well under a second (and zoom/pan is smooth) — the colors for the shown FOV, the cell gallery, exports, and ROIs are computed on demand instead of pre-coloring every cell in every FOV up front. The status bar now shows the busy state while colors are applied. A follow-up (#115 reply 2) removed the last slowdown: enabling continuous coloring or changing the colormap used to refresh the whole viewer several times over (a hidden feedback loop kept re-drawing), so each action now redraws exactly once.
- Color cell masks by a continuous variable (#115): the **Mask Painter** now has a **Continuous** mode (next to the usual **Categorical** mode) that colors masks with a **gradient** based on a numeric column from the cell table (e.g. a protein abundance or feature value). Pick the column and a colormap, optionally apply an **arcsinh** transform, and set the value range (auto 1–99 percentile or manual), transparency, and fill/outline — a colorbar shows the current mapping. The range is computed across all FOVs so the same value is the same color everywhere, and the gradient is reflected in the cell gallery, batch exports, and saved ROIs. Continuous settings save and load with your mask-color palettes.
- Reconnected the heatmap's plugin links after the histogram/scatter split (#114): in the heatmap's **Linked plugins** tab the old **Chart** checkbox is now split into **Scatter plot** and **Histogram** checkboxes. Selecting a cluster in the heatmap now (a) colours and highlights those cells in the **Scatter plot** plugin and (b) overlays the cluster's cells as a **"Selected"** distribution on every channel in the **Histogram** plugin — the histogram response that had been missing since the plugins were separated.
