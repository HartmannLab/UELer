...
- Structure handling is **descriptor-first**: an optional JSON descriptor maps the study files onto FOVs/channels/masks; auto-detection covers the two clean layouts that mirror the local modes (folder-per-FOV, OME-TIFF-per-FOV). Reads use **byte-range streaming** for pyramidal OME-TIFFs and a transparent **download-once cache** for everything else (the realistic path for single-resolution MIBI TIFFs). Reference dataset: `S-BIAD2557` (folder-per-FOV MIBI; 562 FOVs) — verified end-to-end (base URL resolved, channels listed, one channel streamed to cache, mask prefetched).
