### v0.4.2
**Reply to #118 (cont.) — Reset the scatter view once the frontend mounts**
- Construction-time scale (below) set the axis domain but the plots still opened mis-framed — the **right-hand y-axis ticks/labels were hidden** until the user clicked **reset view**. jscatter fits the camera and lays out the axes on the *frontend*, and on first mount that layout did not happen. **Fix (the developer's core suggestion — call the reset-view callback once the plot is rendered):** `ScatterPlotWidget` now observes the jscatter widget's read-only `dom_element_id` trait — which the frontend writes once it has mounted the widget in the DOM (the standard anywidget "frontend ready" signal) — and calls `widget.reset_view()` when it fires, reproducing the reset-view click automatically so every plot opens correctly framed (axes/ticks drawn, consistent scale) with no manual click. The observer is torn down in `dispose()`.
- Tests: new `ScatterWidgetResetViewTests` (reset_view fires when `dom_element_id` is written). All targeted scatter/footer tests pass; full-suite failure/error set identical to the `develop` baseline (no new failures/errors); live rendering to be confirmed in the notebook. See `dev_note/issue_tracking/issue118_scatter_crop_wide_layout.md`.
