...
__all__ = ["RunFlowsom", "UiComponent", "Data", "linked_controls"]

try:  # pragma: no cover - optional dependency
    from pyFlowSOM import map_data_to_nodes, som
except Exception:  # pragma: no cover - FlowSOM plugin requires pyFlowSOM at runtime
    def _missing_pyflowsom(*_args, **_kwargs):
        raise ImportError("pyFlowSOM is required to run the FlowSOM plugin")

    map_data_to_nodes = _missing_pyflowsom  # type: ignore[assignment]
    som = _missing_pyflowsom  # type: ignore[assignment]
from scipy.cluster.hierarchy import cut_tree, dendrogram, linkage

from ueler.image_utils import (  # type: ignore[import]
    color_one_image,
...
class RunFlowsom(PluginBase):
    ...
    def run_flowsom(self, b):
        ...
        clusters, _ = map_data_to_nodes(node_output, flowsom_array)
...
class linked_controls:
    def __init__(self):
        # Placeholder to maintain plugin interface parity with legacy widgets.
        pass